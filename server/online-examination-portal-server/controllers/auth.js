...
const register = async (req, res) => {
    ...
    // res.send(`/activate/${userType}/${token}`);
    res.status(StatusCodes.ACCEPTED).json({ user: { userID: user._id, name: user.name, email: user.email }, msg: 'Registration Successful' , lnk:`/activate/${userType}/${token}`})
    // res.redirect(`http://localhost:3000/activate/${this.userType}/${token}`)
}
...
const login = async (req, res) => {
    ...
    //     // res.send(`http://localhost:3000/activate/${this.userType}/${token}`)
    // }    
...
const changePassword = async (req, res) => {
    ...
    const token = jwt.sign({ userID: user._id, email: user.email, password: user.password }, process.env.JWT_SECRET, { expiresIn: 2 * 24 * 60 * 60 })
